...
let migrationStatus = {
  customers: { total: 0, migrated: 0, errors: [] as any[] },
  quotes: { total: 0, migrated: 0, errors: [] as any[] },
  invoices: { total: 0, migrated: 0, errors: [] as any[] },
  shareLinks: { total: 0, migrated: 0, errors: [] as any[] },
  shareTokens: { total: 0, migrated: 0, errors: [] as any[] },
  emailHistory: { total: 0, migrated: 0, errors: [] as any[] },
  calendarEvents: { total: 0, migrated: 0, errors: [] as any[] }
};
...
async function batchInsert(table: string, data: any[], batchSize = 100) {
...
async function migrateInvoices() {
  ...
  try {
    ...
    // Get customer and quote ID mappings
    const { data: customerMapping } = await supabase
      .from('customers')
      .select('id, firebase_id');
    ...
    const quoteMap = new Map(
      quoteMapping?.map(q => [q.firebase_id, q.id]) || []
    );
    
    invoicesSnapshot.forEach((doc) => {
      const data = doc.data();
      const customerId = customerMap.get(data.customerId);
      const quoteId = quoteMap.get(data.quoteId);
      
      if (!customerId) {
        console.warn(`⚠️  No customer found for invoice ${doc.id}`);
        return;
      }
      ...
      invoices.push({
        ...
        invoice_number: data.invoiceNumber || `INV-${doc.id.substring(0, 8)}`,
    ...
    migrationStatus.invoices.total = invoices.length;
    console.log(`📦 Found ${invoices.length} invoices to migrate`);
    
    if (invoices.length > 0) {
      await batchInsert('invoices', invoices);
      migrationStatus.invoices.migrated = invoices.length;
    }
    
    console.log('✅ Invoices migration completed');
  } catch (error) {
    console.error('❌ Error migrating invoices:', error);
    migrationStatus.invoices.errors.push(error);
  }
}

// Migrate share links
async function migrateShareLinks() {
  console.log('\n🚀 Migrating share links...');
  
  try {
    const shareLinksSnapshot = await getDocs(collection(firestore, 'shareLinks'));
    const shareLinks = [] as any[];
    ...
    shareLinksSnapshot.forEach((doc) => {
      const data = doc.data();
      const customerId = customerMap.get(data.customerId);
      const quoteId = data.quoteId ? quoteMap.get(data.quoteId) : null;
      
      if (!customerId) {
        console.warn(`⚠️  No customer found for share link ${doc.id}`);
        return;
      }
      ...
      shareLinks.push({
        ...
        expires_at: data.expiresAt ? new Date(data.expiresAt).toISOString() : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        created_by: data.createdBy || 'system',
        used_at: data.usedAt ? new Date(data.usedAt).toISOString() : null,
        arbeitsschein_html: data.arbeitsscheinHTML || null,
        arbeitsschein_data: data.arbeitsscheinData || null,
        created_at: data.createdAt ? new Date(data.createdAt).toISOString() : new Date().toISOString()
      });
    });
    
    migrationStatus.shareLinks.total = shareLinks.length;
    console.log(`📦 Found ${shareLinks.length} share links to migrate`);
    
    if (shareLinks.length > 0) {
      await batchInsert('share_links', shareLinks);
      migrationStatus.shareLinks.migrated = shareLinks.length;
    }
    
    console.log('✅ Share links migration completed');
  } catch (error) {
    console.error('❌ Error migrating share links:', error);
    migrationStatus.shareLinks.errors.push(error);
  }
}
...
async function migrateEmailHistory() {
  ...
  try {
    const emailHistorySnapshot = await getDocs(collection(firestore, 'emailHistory'));
    const emailHistory = [] as any[];
    ...
    emailHistorySnapshot.forEach((doc) => {
      const data = doc.data();
      const customerId = customerMap.get(data.customerId);
      
      if (!customerId) {
        console.warn(`⚠️  No customer found for email history ${doc.id}`);
        return;
      }
      ...
      emailHistory.push({
        ...
        sent_at: data.sentAt ? new Date(data.sentAt).toISOString() : new Date().toISOString(),
        body: data.body || '',
    ...
    migrationStatus.emailHistory.total = emailHistory.length;
    console.log(`📦 Found ${emailHistory.length} email history records to migrate`);
    
    if (emailHistory.length > 0) {
      await batchInsert('email_history', emailHistory);
      migrationStatus.emailHistory.migrated = emailHistory.length;
    }
    
    console.log('✅ Email history migration completed');
  } catch (error) {
    console.error('❌ Error migrating email history:', error);
    migrationStatus.emailHistory.errors.push(error);
  }
}
...
async function migrateCalendarEvents() {
  ...
  try {
    const calendarEventsSnapshot = await getDocs(collection(firestore, 'calendarEvents'));
    ...
    calendarEventsSnapshot.forEach((doc) => {
      ...
      if (!customerId) {
        console.warn(`⚠️  No customer found for calendar event ${doc.id}`);
        return;
      }
      ...
      calendarEvents.push({
        ...
        start_date: data.start ? new Date(data.start).toISOString() : new Date().toISOString(),
        end_date: data.end ? new Date(data.end).toISOString() : new Date().toISOString(),
        type: data.type || 'move',
    ...
    migrationStatus.calendarEvents.total = calendarEvents.length;
    console.log(`📦 Found ${calendarEvents.length} calendar events to migrate`);
    
    if (calendarEvents.length > 0) {
      await batchInsert('calendar_events', calendarEvents);
      migrationStatus.calendarEvents.migrated = calendarEvents.length;
    }
    
    console.log('✅ Calendar events migration completed');
  } catch (error) {
    console.error('❌ Error migrating calendar events:', error);
    migrationStatus.calendarEvents.errors.push(error);
  }
}

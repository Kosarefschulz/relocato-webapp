...
exports.checkEmails = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      const processedEmails = await checkIONOSInbox(folder, limit, testMode);
 ...
 */
async function checkIONOSInbox(folderName, maxEmails, testMode) {
  ...
  return new Promise((resolve, reject) => {
    ...
    imap.once('ready', () => {
      ...
      async function processFolder(box, currentFolder) {
        ...
        let searchCriteria;
        if (testMode) {
          // Test-Modus: Alle E-Mails von ImmoScout24 oder Umzug365
          searchCriteria = [
            ['OR',
              ['OR',
                ['FROM', 'immoscout24'],
